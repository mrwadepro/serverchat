 ...
 ************************/
	...
	public static byte[] encrypt(byte[] stringToEncrypt, SecretKeySpec secretKey)
	{
		Cipher cipher = null;
		try
		{
		...
		try 
		{
			encry = cipher.doFinal(stringToEncrypt);
		} 
    ...
    public static byte[] decrypt(byte[] stringToDecrypt, SecretKeySpec secretKey)
    {
    	Cipher cipher = null;
		try 
		{
        ...
        try
        {
			data = cipher.doFinal(stringToDecrypt);
		} 
        catch (IllegalBlockSizeException | BadPaddingException e)
        {
			e.printStackTrace();
